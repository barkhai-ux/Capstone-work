...
function ResultChart({ result }: { result: QueryResult }) {
  const { chartConfig, rows, insight } = result;
  ...
  return (
    <div>
      ...
      {insight && (
        <div className="mt-3 mx-4 mb-3 flex items-start gap-2 bg-blue-50 rounded-lg px-3 py-2.5">
                            ...
                            <div>
                              ...
                              <div className="border-t border-gray-100">
                                <button
                                  ...
                                  className="w-full flex items-center justify-center gap-1.5 px-4 py-2 text-[11px] text-gray-400 hover:text-gray-600 hover:bg-gray-50 transition-colors"
                                >
                                  <svg className={`w-3 h-3 transition-transform ${expandedTables.has(msg.id) ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                                    <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" />
                                  </svg>
                                  {expandedTables.has(msg.id) ? 'Hide data' : 'Show data'}
                                </button>
                                {expandedTables.has(msg.id) && (
                                  <div className="overflow-x-auto max-h-60 border-t border-gray-100">
